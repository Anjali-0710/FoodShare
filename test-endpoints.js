 ...
 * Verifies backend REST and AI APIs with verbose error diagnostics.
 */
...
async function safeFetch(url, options = {}) {
  ...
  console.log(`  [HTTP ${method}] Requesting: ${url}`);
  try {
    ...
    if (!res.ok) {
      ...
      console.error(`     • Response Body: ${bodyText || '(empty)'}`);
      throw new Error(`HTTP ${res.status} ${res.statusText} for ${url}: ${bodyText}`);
    }

    return { status: res.status, data: bodyData || bodyText };
  } catch (err) {
    if (!err.message.startsWith('HTTP ')) {
      console.error(`  ❌ Network / Fetch Exception!`);
      console.error(`     • Requested URL: ${url}`);
      console.error(`     • Method:        ${method}`);
...
async function waitForServerReadiness(maxRetries = 15, delayMs = 2000) {
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      console.warn(`  ⚠️ Health check failed (${err.message}), retrying in ${delayMs}ms...`);
    }
    await new Promise((resolve) => setTimeout(resolve, delayMs));
  }

  throw new Error(`Server at ${healthUrl} failed to respond after ${maxRetries} attempts.`);
}

async function runTests() {
  console.log('===========================================================');
  console.log('🧪 FOODREACH AI — VERBOSE API INTEGRATION TEST SUITE');
  console.log('===========================================================');
